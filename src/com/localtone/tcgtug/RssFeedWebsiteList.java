...
public class RssFeedWebsiteList extends ListActivity implements OnClickListener {

	private static List<RssFeedWebsite> feedItems = RssFeedWebsite.feedItems;
		
	@Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);

        Iterator<RssFeedWebsite> fi = feedItems.iterator();
        
        String[]dd = new String [feedItems.size()]; 
        
        int i = 0;
        while(fi.hasNext()) {
        	RssFeedWebsite rss = (RssFeedWebsite)fi.next();
        	dd[i] = rss.getTitle();
        	i++;
        }
